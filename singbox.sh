...
# - 兼容资产命名：
#     sing-box-<ver>-linux-<arch>.tar.gz   (新命名，如 1.13.0)
#     sing-box-linux-<arch>.tar.gz         (旧命名)
# ============================================================
...
# 关键：智能匹配 tar.gz 资产 URL（兼容新旧命名）
json_pick_linux_tarball_url() {
  local json="$1" arch="$2"
  ...
  if command -v jq >/dev/null 2>&1; then
    ...
    u="$(jq -r --arg a "$arch" '
      .assets
      | map(select(.name | test("^sing-box-[0-9]+\\.[0-9]+\\.[0-9]+-linux-"+$a+"\\.tar\\.gz$")))
    ...
    u="$(jq -r --arg a "$arch" '
      .assets
      | map(select(.name == ("sing-box-linux-"+$a+".tar.gz")))
      | .[0].browser_download_url // empty
    ' "$json")"
    [[ -n "$u" ]] && { echo "$u"; return 0; }
    ...
    u="$(jq -r --arg a "$arch" '
      .assets
      | map(select(.name | test("linux-"+$a+"\\.tar\\.gz$")))
      | .[0].browser_download_url // empty
    ' "$json")"
    [[ -n "$u" ]] && { echo "$u"; return 0; }
    ...
    local urls u
    urls="$(grep -oE '"browser_download_url":\s*"[^"]+"' "$json" | sed -E 's/.*"([^"]+)".*/\1/' || true)"

    u="$(echo "$urls" | grep -E "sing-box-[0-9]+\.[0-9]+\.[0-9]+-linux-${arch}\.tar\.gz$" | head -n1 || true)"
    [[ -n "$u" ]] && { echo "$u"; return 0; }

    u="$(echo "$urls" | grep -E "sing-box-linux-${arch}\.tar\.gz$" | head -n1 || true)"
    [[ -n "$u" ]] && { echo "$u"; return 0; }

    u="$(echo "$urls" | grep -E "linux-${arch}\.tar\.gz$" | head -n1 || true)"
    [[ -n "$u" ]] && { echo "$u"; return 0; }
...
download_and_install_binary() {
  local mode="$1" ver="${2:-}"
  local arch tmpdir json json2 tag url asset tarball extracted_bin
  ...
  url="$(json_pick_linux_tarball_url "$json" "$arch" || true)"
  [[ -n "$url" ]] || fail "未找到 linux-${arch} 的 tar.gz 安装包（release：${tag}）"
  ...
  extracted_bin="$(find "$tmpdir" -maxdepth 6 -type f -name "$BIN_NAME" -perm -u+x | head -n1 || true)"
...
# ------------------ 服务管理 ------------------
service_exists() { sudo_cmd systemctl list-unit-files | grep -q "^${SERVICE_NAME}\.service"; }
...
service_start()   { service_exists || fail "未安装服务，请先安装。"; sudo_cmd systemctl start "$SERVICE_NAME"; ok "已启动服务"; }
service_stop()    { service_exists || fail "未安装服务，请先安装。"; sudo_cmd systemctl stop "$SERVICE_NAME" || true; ok "已停止服务"; }
service_restart() { service_exists || fail "未安装服务，请先安装。"; sudo_cmd systemctl restart "$SERVICE_NAME"; ok "已重启服务"; }
service_enable()  { service_exists || fail "未安装服务，请先安装。"; sudo_cmd systemctl enable "$SERVICE_NAME" >/dev/null || true; ok "已设置开机自启"; }
service_disable() { service_exists || fail "未安装服务，请先安装。"; sudo_cmd systemctl disable "$SERVICE_NAME" >/dev/null || true; ok "已取消开机自启"; }
...
# ------------------ 配置闭环：编辑/校验/应用 ------------------
pick_editor() {
  if [[ -n "${EDITOR:-}" ]] && command -v "$EDITOR" >/dev/null 2>&1; then echo "$EDITOR"; return 0; fi
  for e in nano vim vi; do command -v "$e" >/dev/null 2>&1 && { echo "$e"; return 0; }; done
  fail "未找到可用编辑器（nano/vim/vi）。请先安装一个或设置 EDITOR。"
}
...
config_check() {
  ...
  "$BIN_PATH" check -c "$CONF_FILE" && ok "配置校验通过" || { warn "配置校验失败（请根据输出修正）"; return 1; }
}

config_apply_restart() {
  config_check || { warn "校验未通过，已取消应用"; return 1; }
  service_exists || { warn "服务尚未安装，将仅完成校验"; return 0; }
  info "应用配置：重启服务使配置生效..."
  sudo_cmd systemctl restart "$SERVICE_NAME"
  sudo_cmd systemctl is-active --quiet "$SERVICE_NAME" && ok "已重启且服务运行正常" || warn "重启后服务未运行：journalctl -u ${SERVICE_NAME} -n 200 --no-pager"
}
...
install_or_upgrade_flow() {
  ...
  sudo_cmd systemctl is-active --quiet "$SERVICE_NAME" && ok "服务已运行" || warn "服务未运行：建议 配置管理->编辑->校验并应用"
}
...
uninstall_flow() {
  ...
  [[ -f "$SERVICE_FILE" ]] && { info "删除服务文件：${SERVICE_FILE}"; sudo_cmd rm -f "$SERVICE_FILE"; sudo_cmd systemctl daemon-reload; }
  [[ -f "$BIN_PATH" ]] && { info "删除二进制：${BIN_PATH}"; sudo_cmd rm -f "$BIN_PATH"; }

  read -rp "是否删除配置/数据目录（${CONF_DIR} ${STATE_DIR} ${LOG_DIR}）？(y/N)： " delconf
  [[ "${delconf:-N}" =~ ^[Yy]$ ]] && { sudo_cmd rm -rf "$CONF_DIR" "$STATE_DIR" "$LOG_DIR"; ok "已删除配置/数据目录"; } || info "保留配置/数据目录"

  read -rp "是否删除系统用户 ${RUN_USER}？(y/N)： " deluser
  if [[ "${deluser:-N}" =~ ^[Yy]$ ]]; then
    id -u "$RUN_USER" >/dev/null 2>&1 && { sudo_cmd userdel "$RUN_USER" >/dev/null 2>&1 || true; ok "已删除用户：${RUN_USER}"; }
  else
    info "保留系统用户"
  fi
...
show_status() {
  echo
  info "当前状态概览："
  [[ -x "$BIN_PATH" ]] && { ok "二进制：${BIN_PATH}"; "$BIN_PATH" version || true; } || warn "未安装二进制：${BIN_PATH}"

  if service_exists; then
    sudo_cmd systemctl is-active --quiet "$SERVICE_NAME" && ok "服务：${SERVICE_NAME}（运行中）" || warn "服务：${SERVICE_NAME}（未运行）"
  else
    warn "未发现 systemd 服务：${SERVICE_NAME}"
  fi

  [[ -f "$CONF_FILE" ]] && ok "配置：${CONF_FILE}" || warn "未发现配置文件：${CONF_FILE}"
  echo
}
...
main() {
  ...
  if [[ "${EUID:-$(id -u)}" -ne 0 ]]; then USE_SUDO=1; else USE_SUDO=0; fi
  ...
  while true; do
    ...
    case "$choice" in
      ...
      2)
        ...
        install_or_upgrade_flow "version" "$v"; pause
        ;;
      3)
        warn "Beta/预发布可能不稳定，仅建议测试环境使用"
        read -rp "确认继续？(y/N)： " yn
        [[ "${yn:-N}" =~ ^[Yy]$ ]] || { info "已取消"; continue; }
        install_or_upgrade_flow "beta"; pause
        ;;
      4) config_menu_loop ;;
      5) service_menu_loop ;;
      6) show_status; pause ;;
